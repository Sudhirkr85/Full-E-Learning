...
*   **Lesson Player & Progress Tracking**: Immersive video/article lesson interface that registers completed and paused states, updating course progress percentages instantly. Upgraded to a premium, distraction-free **Plyr Custom Video Player** that streams YouTube assets securely via high-performance CDN assets (zero local package bloat). It overlays beautiful, customized dark Indigo playback controls, hides native ads, branding overlays, and recommends, and implements native mobile **Landscape Screen Orientation locking** programmatically when full-screen triggers to ensure absolute comfort on mobile viewports.
*   **Tests & Quizzes (Assessments)**: Graded exams, practice tests, and quizzes. Supports single-choice, multiple-choice, true/false, and short-answer questions with auto-grading scripts. Upgraded to a state-of-the-art **Immersive Full-Screen Quiz Assessment Designer** featuring dynamic center layout workspaces, focused glassmorphic sub-modal creators, custom-scaled standard Tailwind selection controls, and a fully **Timed Online Quiz Portal** inside the student classroom interface complete with case-sensitivity grading, local timing trackers, and automatic database evaluations.
*   **Layout-Independent Viewport Portal Popups**: Replaced all native browser alert/confirm dialogues and tracking shipment overlays with a premium, layout-independent React Portal system (`createPortal` to `document.body`). This guarantees that all modal dialogues, confirm boxes, and dispatch cards escape parent layout containment clips, remain perfectly centered in the active browser viewport, and automatically trigger focus and smooth-scroll on primary action CTAs (Cancel/Confirm) for perfect ergonomics across long desktop or mobile pages.
*   **Robust Section & Lesson Order Indexes**: Swapped out `count()` calculations for robust, database-safe calculations based on `max(orderIndex) + 1`, completely eliminating `Unique constraint failed (sectionId, orderIndex)` SQL crashes during module updates.
*   **Production Auth.js v5 Secure Cookie Middleware Protection**: Configured custom Next.js Middleware route guards to dynamically resolve `secureCookie` and `cookieName` (e.g. `__Secure-authjs.session-token` on HTTPS) based on the browser protocol. This resolves desynchronized login session states and prevents infinite production redirect loops on Vercel.
*   **PDF Curriculum Uploader**: Built a local, drag-and-drop secure file uploader directly in the curriculum content panel that uploads PDF playbook assets to Cloudflare R2 on save.
