...
*   **Courses & Lesson Navigation**: Dynamic public course catalog at `/courses` completely redesigned with a high-fidelity cinematic dark theme (`bg-[#0a0a0f]`), `.bg-grid-cyber` mesh overlays, soft indigo radial glows, and dark glassmorphic category filter pills. Cards feature immersive `h-48` thumbnail banners, overlay titles, translucent category badges, teacher metadata, and precise INR pricing grids (strikethroughs, "% OFF" emerald badges, and green "Free" badges). Employs dynamic action buttons directing students to "Continue Learning" (filled indigo) or "View Details" (ghost border) with hardware-accelerated translation lifts (`hover:-translate-y-1.5`). Course detail page shows 2-column layout (desktop) with sticky price card. Left side: thumbnail, meta, teacher info, description with read-more, curriculum accordion with preview/locked lessons. Right side: sticky card with INR pricing, discount badge, enrollment CTA based on login and enrollment state, and course includes section.
