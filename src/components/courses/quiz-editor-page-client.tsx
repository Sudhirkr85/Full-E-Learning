...
type QuizEditorPageClientProps = {
  courseId: string;
  lesson: any;
  test: any;
  role: "ADMIN" | "TEACHER";
...
export function QuizEditorPageClient({
  courseId,
  lesson,
  test,
  role
}: QuizEditorPageClientProps) {
  ...
  const backUrl = role === "ADMIN" 
    ? `/admin/courses/${courseId}`
    : `/teacher/courses/${courseId}`;

  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 py-6 min-h-screen bg-[#0a0a0f] text-slate-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/10 pb-6">
        <div className="space-y-1">
          <Button 
            ...
            variant="ghost" 
            className="p-0 text-xs font-semibold text-slate-400 hover:text-white transition flex items-center gap-1.5 mb-2 hover:bg-transparent"
          >
            <ArrowLeft className="h-3.5 w-3.5" /> Back to Course Editor
          </Button>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest font-mono bg-indigo-500/10 px-2 py-0.5 rounded">
              Quiz Assessment Designer
            </span>
          </div>
          <h1 className="text-2xl font-black text-white tracking-tight">{lesson.title}</h1>
          {test?.description && (
            <p className="text-xs text-slate-400 max-w-2xl">{test.description}</p>
          )}
        </div>
      </div>
      ...
      <div className="py-4">
        {activeSubTab === "settings" && (
          <form onSubmit={handleSaveSettings} className="space-y-6 max-w-3xl bg-[#0f0f18] border border-white/10 rounded-2xl p-6 shadow-xl">
          ...
          <div className="space-y-6">
            ...
            {test?.questions && test.questions.length > 0 ? (
              <div className="space-y-3">
                {test.questions.map((q: any, index: number) => (
                  <div key={q.id} className="p-5 rounded-2xl border border-white/5 bg-[#0f0f18] hover:bg-white/[0.02] hover:border-white/10 transition-all flex items-start justify-between gap-6 shadow-sm">
              ...
              <div className="p-16 border border-dashed border-white/10 rounded-2xl text-center space-y-4 max-w-xl mx-auto my-12 bg-[#0f0f18]">
            ...
            {isEditorOpen && (
              <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
                <div className="bg-[#0b0c16] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl relative flex flex-col h-fit max-h-[85vh] animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <div className="px-5 py-4 border-t border-white/10 flex gap-2 justify-end bg-[#0f0f18]">
                    <Button 
                      type="button" 
                      onClick={handleCloseEditor} 
