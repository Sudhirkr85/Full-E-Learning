...
export function CourseDashboardClient({
  ...
  const handleCreateSection = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await createSectionAction(course.id, newSectionTitle);
      if (res.error) {
        showAlert(res.error);
      } else {
        setNewSectionTitle("");
  ...
  const handleCreateLesson = async (e: React.FormEvent, sectionId: string) => {
    ...
    startTransition(async () => {
      const res = await createLessonAction(
        ...
        sectionId,
        newContentTitle,
        contentType === "QUIZ" ? "QUIZ" as any : contentType === "ARTICLE" ? "ARTICLE" : "VIDEO",
        {
          description: newContentDesc.trim() || undefined,
          youtubeUrl,
  ...
  const handleDeleteSection = async (sectionId: string) => {
    showConfirm(
      ...
      async () => {
        startTransition(async () => {
          await deleteSectionAction(course.id, sectionId);
        });
      }
    );
  };
  ...
  const handleDeleteLesson = async (lessonId: string) => {
    showConfirm(
      ...
      async () => {
        startTransition(async () => {
          await deleteLessonAction(course.id, lessonId);
        });
      }
    );
  };
