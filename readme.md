...
*   **Enrollment & Store Order Dashboard Improvements**:
    *   **Accurate Course Revenue Metrics**: Fixed course revenue metrics calculating as ₹0 by querying the `Enrollment` table directly for `amountPaid` instead of relying on the `Payment` table (which is only populated for store orders).
...
*   **LMS Classroom & Assessment Enhancements**:
    ...
    *   **Quiz Review Button Fixed**: Fixed the quiz attempt "Review" buttons in the Attempt Roster not opening the review screen. Root cause was Next.js App Router caching not re-fetching server props when only query parameters changed via `router.push()`. Resolved by switching all quiz navigation (Review, Submit, Retake/Go Back) to use `router.push()` + `router.refresh()` and adding a `useSearchParams` listener with a `useEffect` to automatically trigger `router.refresh()` whenever the `attemptId` query param changes — making browser Back/Forward buttons work correctly too.
    *   **Sequential Back Button Navigation**: Fixed the back button in the lesson player and classroom player headers to navigate one logical step back instead of jumping all the way to the course landing page. When reviewing a quiz attempt, the back button now correctly returns to the quiz overview screen first; only then does a second click return to the course page.

*   **UI/UX Refinements**:
    *   **Direct Course Checkout Drawer**: Integrated a premium interactive Checkout slide-out Drawer when clicking 'Buy Now' / 'Enroll' on course cards or course detail sections. Styled to match the store cart drawer layout using React Portals (`createPortal`) to prevent containment clipping, and incorporates the same conditional **'Have a Coupon? [Apply Coupon]'** expander input flow before initiating payments via Razorpay.
    ...
    *   **Add to Calendar Button Color**: Fixed the "Add to Calendar" button on the Live Class upcoming screen. The button previously used an `outline` variant which rendered with a near-white background and very low-contrast text, making it unreadable against the dark theme. Replaced with a solid `bg-indigo-600` styled button with white text and an indigo glow shadow for clear visibility.
...
*   **Bug Fixes & System Stability**:
    ...
    *   **PDF Proxy 403 Fix**: Resolved a recurring HTTP 403 "Unauthorized" error returned by `/api/pdf-proxy` when loading PDF lesson content. Root causes were: (1) strict exact-match DB lookup failing due to URL encoding differences between the stored `r2AssetUrl` and the proxy query param, (2) Admin/Teacher users unnecessarily going through DB enrollment checks and failing, (3) preview lessons being blocked despite being publicly accessible. Fixed by: normalizing the URL with `decodeURIComponent()` before lookup, trying both raw and decoded URLs via an `OR` query, short-circuiting all DB checks for Admin/Teacher users to immediately stream the PDF, and granting any authenticated user access to `isPreview = true` lessons. Also added `Cache-Control: private, max-age=3600` to reduce repeated proxy calls.
    *   **Desktop Drag-to-Scroll (Hand Tool) Support**: Equipped the [`PdfReader`](file:///d:/Work/E-Learning/src/components/pdf-reader/PdfReader.tsx) with desktop click-and-drag panning. When using the default view tool, users can hold down their left mouse button and drag to pan both vertically and horizontally. This uses the CSS `grab` and `grabbing` cursors to represent active states without degrading document render quality or scale.
    *   **Dynamic Reviews Section Visibility**: Updated [`course-reviews-client.tsx`](file:///d:/Work/E-Learning/src/app/(public)/courses/[slug]/course-reviews-client.tsx) to hide the entire reviews and feedback section completely if there are no reviews yet and the current visitor is either a guest or a student who is not enrolled (and thus cannot write a review). Enrolled students will still see the section with a "Write a Review" button.
    *   **All-Devices Checkout Descriptor**: Replaced the restrictive label `"Access on mobile & desktop"` inside the course page's enrollment widget in [`page.tsx`](file:///d:/Work/E-Learning/src/app/(public)/courses/[slug]/page.tsx) with `"Access on all devices"` to accurately reflect that the platform is fully responsive and accessible on tablets, desktops, and mobiles alike.
