...
export function RegisterForm({ errorMessage }: RegisterFormProps) {
  ...
  const [passwordStrength, setPasswordStrength] = useState({ score: 0, label: "Empty", color: "bg-slate-800" });
  ...
  useEffect(() => {
    if (!password) {
      setPasswordStrength({ score: 0, label: "Empty", color: "bg-slate-800" });
      return;
    }
    ...
    <div className="w-full space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          {password && (
            <div className="space-y-1.5 pt-1 animate-in fade-in duration-200">
              ...
              <div className="h-1.5 w-full rounded-full bg-slate-950 overflow-hidden flex gap-0.5">
                {[1, 2, 3, 4].map((step) => (
                  <div
                    key={step}
                    className={cn(
        ...
        <div className="space-y-1.5">
          ...
          {/* Confirm Password Match Visualizer */}
          {confirmPassword && (
